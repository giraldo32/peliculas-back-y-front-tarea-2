...
const validarNombreTipoUnico = async (req, res, next) => {
    try {
        ...
        const existente = await Tipo.findOne(filtro)
        if (existente) {
            return res.status(400).json({ msj: 'Tipo ya existe' })
        }
        next()
    } catch (error) {
        console.log(error)
...
module.exports = validarNombreTipoUnico;
