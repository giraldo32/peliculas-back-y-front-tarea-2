...
const validarNombreProductoraUnico = async (req, res, next) => {
    try {
        ...
        const existente = await Productora.findOne(filtro)
        if (existente) {
            return res.status(400).json({ msj: 'Productora ya existe' })
        }
        next()
    } catch (error) {
        console.log(error)
...
module.exports = validarNombreProductoraUnico;
